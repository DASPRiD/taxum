...
describe("http:extension", () => {
    describe("ExtensionKey", () => {
        ...
        it("toJSON returns string value", () => {
            const key = new ExtensionKey<string>("key");
            assert.equal(key.toJSON(), "key");
        });

        it("custom inspect returns string value", () => {
            const key = new ExtensionKey<string>("key");
            assert.equal(util.inspect(key), "key");
        });
    });
    ...
    describe("Extensions", () => {
        ...
        it("toJSON returns object value", () => {
            const key1 = new ExtensionKey<string>("foo");
            const key2 = new ExtensionKey<string>("bar");
            const extensions = new Extensions();
            extensions.insert(key1, "value");
            extensions.insert(key2, "other");

            assert.deepEqual(extensions.toJSON(), {
                foo: "value",
                bar: "other",
            });
        });
        ...
        it("custom inspect returns string value", () => {
            ...
            assert.equal(util.inspect(extensions), "{ foo => 'value', bar => 'other' }");
        });
    });
});
